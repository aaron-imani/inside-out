...
temperature = float(env_loader.MODEL_TEMPERATURE)
using_open_source = env_loader.USE_OPEN_SOURCE == "1"
raw_model_name = env_loader.MODEL_NAME
...
def get_query_cost(input_token_count, output_token_count, model="o1"):
    """Returns the cost of a query based on the input and output token count.
    Based on pricing for OpenAI's GPT-4 model.
    https://platform.openai.com/docs/pricing"""

    input_token_count /= 1e6
    output_token_count /= 1e6

    if model == "o1":
        return input_token_count * 15 + output_token_count * 60
...
def is_instruction_tuned_model():
    ...
    try:
        ...
        print(colored("Error checking instruction tuning:", "red"), str(e))
        print(colored("Instruction-tuned:", "blue"), "Unknown")
        return False
